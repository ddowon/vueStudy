<template>
	<span style="width:235px;" class="selectbox_wrap" :class="{ 'active': isListActive }" v-if="categories">
		<a href="#" class="select_result" title="검색 조건 선택" style="width: 194px;" @click.prevent="toggleBox">
			다른 회차 선택하기
			<span class="ico-select"></span>
		</a>
		<ul class="sel_list">
			<!-- <li v-for="item in categories" :key="item.times" :class="{ 'on': currentTimes === item.times }">
			...
			<!--
				...
				// 마찬가지로 active class도 현재 route에 따라 설정되므로 categories만 남게 됨
